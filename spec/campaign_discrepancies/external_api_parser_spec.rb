...
describe CampaignDiscrepancies::ExternalApiParser do
  ...
  describe '#call' do
    it 'returns campaigns from the external ad service api' do
      ...
      expect(campaigns['ads'][0]['reference']).to eq('1')
      expect(campaigns['ads'][0]['status']).to eq('enabled')
      expect(campaigns['ads'][0]['description']).to eq('Description for campaign 11')
    end
